...
- `installNextcloud($domain, $tz, $clean=false)` — Nextcloud AIO mastercontainer via docker-compose in `/opt/nextcloud-aio`. Four-phase flow:
  ...
  3. **Pull + start** — runs `PullContainerImages.php` then `StartContainers.php` inside the mastercontainer (both silent); progress shown by backgrounding `docker events` filtered to `image pull` and `container create/start` events respectively. Background process killed with `|| true` on `wait` to absorb the SIGTERM exit code under `set -e`.
  ...
  `InstallController::runNextcloudStage()` saves all three captured values to `settings` encrypted: `nextcloud.aio_passphrase`, `nextcloud.admin_password`, `nextcloud.service_username` (`lintune-svc`), `nextcloud.service_password`.
